...
that when $B$ is nonsingular, the results do not match the pseudospectrum of
...
if and only if $B$ is singular and $\delta(A,B,\infty) = \infty$ if and only if
$B$ is nonsingular.  We refer to $\delta(A,B,\sigma)$ as the
\textit{pseudospectral separation} of $\sigma$ from $\Lambda_0(A,B)$.

We will show in the proof of Lemma~\ref{lm:resolvent_bound} that with $B\neq 0$
...
\begin{lemma}
  ...
  If $(A,B)$ is regular, then $\sigma\in\widehat{\C}$ satisfies
  $\sigma\in \Lambda_\epsilon(A,B)$ if and only if
  $\delta(A,B,\sigma) < \epsilon$.
\end{lemma}
\begin{proof}
  We first assume $\sigma\in \C$ with $B\neq 0$ and then $B=0$.  If
  $\delta(A,B,\sigma)< \epsilon$, then there exists $E_0$ with
  $\|E_0\|_2 < \epsilon$ for which $\sigma \in \Lambda_0(A-BE_0, B)$ so that
  $\sigma\in\Lambda_\epsilon(A,B)$.  The matrix $E_0$ is constructed using a
  generalized SVD in the proof of Lemma~\ref{lm:resolvent_bound} below.  If
  $\delta(A,B,\sigma)\geq \epsilon$, then $\|E\|_2 \geq \epsilon$ for all $E$
  with $\sigma\in \Lambda_0(A-BE, B)$ so that
  $\sigma\notin\Lambda_\epsilon(A,B)$.  Assume now that $B=0$.  Regularity
  requires that $A$ be nonsingular.  It follows that $\mcal{E}(A,B,\sigma)$ is
  empty and $\delta(A,B,\sigma)=\infty$, since $\sigma \in \Lambda_0(A-BE, B)$
  would require $(A- BE)\vec{v} = \sigma B\vec{v}$ or $A\vec{v} =\vec{0}$ for
  $\vec{v}\neq \vec{0}$.  Thus the sets $\Lambda_\epsilon(A,B)\cap \C$ and
  $\{\sigma \in \C : \delta(A,B,\sigma) < \epsilon\}$ are both empty.

  For $\sigma = \infty$ and given $\epsilon > 0$, we have already noted that
  $\infty\in \Lambda_\epsilon(A,B)$ if and only if $B$ is singular,
  $\delta(A,B,\infty) = 0<\epsilon$ if and only if $B$ is singular, and
  $\delta(A,B,\infty) = \infty \geq \epsilon$ if and only if $B$ is nonsingular.
  If follows that $\infty\in\Lambda_\epsilon(A,B)$ if and only
  $\delta(A,B,\infty) < \epsilon$.
\end{proof}
It follows that
\begin{equation}
...
\end{equation}
is equivalent to \eqref{eq:pseudo_def}.

It will be helpful to note that the pair $(A-\sigma B,B)$ has a generalized
singular value decomposition
\begin{equation}
  \label{eq:gen-svd}
  A -\sigma B = S D_\sigma U^\H, \eqand
  B = S D_b V^\H,
\end{equation}
where $D_\sigma$ and $D_b$ are diagonal, $S$ is nonsingular, and $U$ and $V$
are unitary, \cite{pasa:81, vloa:76}.  If $\sigma \notin \Lambda_0(A,B)$ so
that $A-\sigma B$ is nonsingular, then the columns of $S$ can be scaled so that
$D_1 = I$.  If $B$ is invertible, then the singular values of
$B^{-1} (A-\sigma B)$ are the diagonal elements $s_i$ for $i=1,2,\ldots, n$ of
$D_b^{-1}D_\sigma$.  These are the generalized singular values of
$(A-\sigma B, B)$.  If $B$ has rank $r$ with
$D_b = D_{b,1}\oplus 0_{n-r\times n-r}$ and
$D_{\sigma} = D_{\sigma,1} \oplus D_{\sigma, 2}$ where $D_{\sigma, 1}$ is
$r\times r$, then the finite generalized singular values of $(A-\sigma B, B)$
are the diagonal elements $s_i$ of $D_{b,1}^{-1} D_{\sigma,1}$ for
...
\end{lemma}
\begin{proof}
  The second and third cases follow immediately from the definition of
  $\delta(A,B,\sigma)$.  We assume the conditions of the first case.  If $B=0$,
  then $\mcal{\mcal{E}}(A,B,\sigma)$ is empty and $\delta(A,B,\sigma) = \infty$
  as required.  Suppose that $B\neq 0$.  The matrix $A-\sigma B$ is nonsingular
  and we consider the generalized singular value decomposition
  \eqref{eq:gen-svd} with $D_1 = I$.  We have
  $(A-\sigma B)^{-1} B = U D_b V^\H$, and without loss of generality, we can
  assume that $D_b = \diag(d_1, \ldots, d_n)$, $d_1 > 0$, and $d_k\geq 0$ is
  nonincreasing for $k=1,2,\ldots, n$.  Clearly
  $A - BE -\sigma B = S (I - D V^\H E U) U^\H$ is singular if and only if
  $(I - D V^\H E U)$ is singular which implies that $\|E\|_2 \geq 1/\|D\|_2$.
  Thus $\|E\|_2 \geq 1/d_1$ for all $E\in\mcal{E}(A,B,\sigma)$ and
  \begin{equation*}
    \delta(A,B,\sigma)
    = \inf_{E \in \mcal{E}(A,B,\sigma)} \|E\|_2
    \geq \frac{1}{d_1}
  ...
  Then
  \begin{equation*}
    A-BE_0 - \sigma B = S (I-\vec{e}_1 \vec{e}_1^\H) U^\H
  \end{equation*}
  is singular so that
  \begin{equation*}
    \delta(A,B,\sigma)
  ...
  \end{equation*}
\end{proof}
It is not difficult to show that if $(A,B)$ is regular, then
\begin{equation*}
  \lim_{\sigma \rightarrow \infty} \frac{1}{\|(A-\sigma B)^{-1} B\|_2} = \infty
\end{equation*}
whether $B$ is singular or not.  If we interpret $1/\|(A-\sigma B)^{-1}B\|_2$
as a limit when $\sigma = \infty$, then we have
  \begin{equation*}
     \delta(A, B, \sigma) 
     = 
     \begin{cases}
...
\begin{proof}
  ...
  We consider the more interesting case $\sigma\in\C$, where $\sigma$ is not a
  generalized eigenvalue.  If $B=0$, then $(A,B)$ has no finite generalized
  eigenvalues and $\delta(A,B,\sigma)=\infty$, which is consistent with
  \eqref{eq:normal_delta}.  So we assume that $B\neq 0$.  Let $V$ be the
