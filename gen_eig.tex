...
In this section, we consider the generalized $\epsilon$-pseudospectrum of
$(A,B)$.  Our motivation is that the pseudospectrum largely determines whether
it is feasible to find a shift $\sigma$ to moderate $K_2(A-\sigma B, B)$.  A
rough summary of where we will end up is that we can expect to have useful
error bounds when $\sigma$ is not in the $\epsilon$-pseudospectrum of $B^{-1}A$
for small $\epsilon$.  However, we do not want to assume that $B$ is
nonsingular.  We will instead reformulate standard results on pseudospectra so
as to avoid inversion of $B$.  The presentation is heavily influenced by
section~2 of \cite{trem:05}.

This approach is direct, but it seems to be distinct from what we have seen
elsewhere.  In \cite{hiti:02}, both $A$ and $B$ are perturbed, which means that
when $B$ is nonsingular, the results do not match the pseudospectrum of
...
$(A,B)$, in a nilpotent block in order to remove them from consideration.  We
do not remove infinite eigenvalues in this way, but we do focus on
$\|(A-\sigma B)^{-1}B\|$ as a replacement for the resolvent norm when defining
generalized pseudospectra.  Several other approaches are described in
\cite{trem:05}, none of which is equivalent to what we consider.
...
We assume that $\|\cdot\|$ represents a vector norm and also the corresponding
induced matrix norm.  We adopt several conventions for dealing with inverses of
singular matrices, infinite generalized eigenvalues, and expressions involving
$\infty$.  Along with $1/\infty = 0$ and $1/0=\infty$, our results are stated
with assuming that
\begin{equation}
  \label{eq:convention-inf-resolvent}
  \text{$\|(A-\sigma B)^{-1}B\| = \infty$ for
    $\sigma\in\Lambda_0(A,B)$},
\end{equation}
and
\begin{equation}
  \label{eq:convention-infinite-shift}  
  \text{$\|(A-\infty B)^{-1}B\| = 0$ for $\infty \notin \Lambda_0(A,B)$.}
\end{equation}
As can be seen from the Weierstrass canonical form \cite{gant:59} of the
regular pair $(A,B)$, these identities are mostly what one would expect as the
limit of $\|(A-\sigma B)^{-1}B\|$ as $\sigma$ approaches a generalized
eigenvalue or as $\sigma$ approaches $\infty$ when
$\infty\notin\Lambda_0(A,B)$.  The exception is that if
$\sigma \in \Lambda_0(A,B)$ and the blocks in the Weierstrass form associated
with the generalized eigenvalue $\infty$ are all $1\times 1$ or $2\times 2$,
then $\|(A-\sigma B)^{-1}B\|$ remains bounded as $\sigma$ approaches $\infty$.
The case $B=0$ is a simple example.  In any identify of the form
$(A-\sigma B)\vec{v} = \vec{u}$, we interpret the case
$(A-\infty B)\vec{v} = \vec{u}$ as equivalent to
$\lim_{\sigma \rightarrow \infty} (A-\sigma B)\vec{v} = \vec{u}$.  That is
\begin{equation}
  \label{eq:convention-inf-dominates}
  \text{$(A-\infty B) \vec{v} =\vec{u}$ is equivalent to $B\vec{v}=\vec{0}$ and $A\vec{v}=\vec{u}$.}
\end{equation}

We will follow \cite{trem:05} in stating and proving the equivalence of several
definitions of the $\epsilon$-pseudospectrum.  The first definition is as follows.
\begin{definition}
  \label{def:pseudo-def-pert1}
  For $0 \leq \epsilon < \infty$,
  we define the $\epsilon$-pseudospectrum of $(A,B)$ by
  \begin{equation}
    \label{eq:pseudo-def-pert1}
    \Lambda_\epsilon(A,B) = \left\{ \sigma \in \widehat{\C} :
      \text{$\sigma \in \Lambda_0(A-BE, B)$ for some $E$
        with $\|E\| \leq \epsilon$} \right\}.
  \end{equation}
\end{definition}
When $B$ happens to be nonsingular, this is easily seen to be the
$\epsilon$-pseudospectrum of $B^{-1}A$ \cite{trem:05}.  Note that we do not
follow then convention from \cite{trem:05} of using the strict inequality
$\|E\| < \epsilon$.  Strict inequalities may be more natural in some
applications, but we will find that the non-strict inequalities simplify the
handling of infinite generalized eigenvalues.

The pair $(A-BE, B)$ has infinite generalized eigenvalues if and only if $B$ is
singular and, hence, if and only if $(A, B)$ has infinite eigenvalues.  It
follows that for any $\epsilon > 0$, $\infty \in \Lambda_\epsilon(A,B)$ if and
only if $\infty \in \Lambda_0(A,B)$.  From this, it can be seen that
perturbations of the form $(A-BE, B)$ do not change the geometric multiplicity
of $\infty$.  However, it can easily be seen that the algebraic multiplicity
can change.  If $Z$ is the $n\times n$ up-shift matrix with ones on the first
superdiagonal, then in the Weierstrass form, $\infty$ is a generalized
eigenvalue of $(I,Z)$ with geometric multiplicity one and algebraic
multiplicity $n$.  However if we set $E=Z^\T$, then $(I-ZZ^\T, Z)$ has
eigenvalue $\infty$ with algebraic multiplicity one and eigenvalue zero with
geometric multiplicity $n-1$.

The next definition of $\Lambda_\epsilon(A,B)$ is the following
\begin{definition}
  \label{def:pseudo-vector}
  For $0\leq \epsilon < \infty$, we define $\Lambda_\epsilon(A,B)$ to be the
  set of all $\sigma \in \widehat{C}$ such that
  \begin{equation*}
    (A-\sigma B) \vec{v} = B\vec{w}
  \end{equation*}
  for some $\vec{v}$ and $\vec{w}$ satisfying $\|\vec{v}\|=1$ and
  $\|\vec{w}\|\leq \epsilon$.
\end{definition}
Finally, we have our third definition.
\begin{definition}
  \label{def:pseudo-resolvent}
  For $0\leq \epsilon < \infty$, we define $\Lambda_\epsilon(A,B)$ to be the
  set of all $\sigma \in \widehat{C}$ such that
  \begin{equation*}
    \|(A-\sigma B)^{-1} B\| \geq \frac{1}{\epsilon}.
  \end{equation*}
\end{definition}
When $\epsilon=0$, $\sigma = \infty$, or $\sigma \in \Lambda_0(A,B)$ (so that
$A-\sigma B$ is singular), all three definitions should be interpreted in terms
of the conventions given above.

\begin{theorem}
  \label{th:def-equivalence}
  Definitions~\ref{def:pseudo-def-pert1}, \ref{def:pseudo-vector}, and
  \ref{def:pseudo-resolvent} are equivalent.
\end{theorem}
\begin{proof}
  We cycle through the conditions in each definition, showing that each
  condition implies the next.  We start with
  Definition~\ref{def:pseudo-def-pert1} for which
  $\sigma \in \Lambda_\epsilon(A,B)$ implies there exists $E$ with
  $\|E\| \leq \epsilon$ and $\sigma \in \Lambda_0(A-BE,B)$.  If
  $\sigma = \infty$, then this implies that there exists $\vec{v}$ with
  $\|\vec{v}\|=1$ and $B\vec{v} = \vec{0}$.  Thus, using
  \eqref{eq:convention-inf-dominates}
  \begin{equation*}
    (A-\sigma B) \vec{v} = \vec{0} = B\vec{w}
  \end{equation*}
  where $\vec{w}=\vec{0}$ so that $\|\vec{w}\|\leq \epsilon$.  If $\sigma\in\C$, then
  there exists $\vec{v}$ with $\|\vec{v}\|=1$ such that $(A-BE)\vec{v} = \sigma B\vec{v}$
  or $(A-\sigma B)\vec{v} = BE\vec{v} = B\vec{w}$, where $\|w\| = \|E\vec{v}\| \leq \epsilon$.

  If we assume that the conditions of Definition~\ref{def:pseudo-vector} hold,
  then we have $\vec{v}$ and $\vec{w}$ with $\|\vec{v}\|=1$,
  $\|\vec{w}\|\leq \epsilon$, and $(A-\sigma B)\vec{v} = B\vec{w}$.  Suppose
  that $\sigma = \infty$.  Then $A\vec{v} - B\vec{w} - \sigma B\vec{v}=\vec{0}$
  and \eqref{eq:convention-inf-dominates} imply $B\vec{v} =\vec{0}$ so that
  $\sigma \in \Lambda_0(A,B)$.  By \eqref{eq:convention-inf-resolvent} we then
  have $\|(A-\sigma B)^{-1}B\|_2 = \infty$ so that
  $\|(A-\sigma B)^{-1}B\|_2 \geq 1/\epsilon$.  If $\sigma \in \C$ and
  $\sigma \notin \Lambda_0(A,B)$ then $A-\sigma B$ is invertible and we have
  $\vec{v} = (A-\sigma B)^{-1} B\vec{w}$ with $\|\vec{v}\| = 1$ and
  $\|\vec{w}\| \leq \epsilon$.  This implies that $\vec{w}\neq \vec{0}$ so that
  $\epsilon > 0$ and that $\|(A-\sigma B)^{-1}B\| \geq 1/\epsilon$.  If $\sigma\in\C$
  and $\sigma \in\Lambda_0(A,B)$, then \eqref{eq:convention-inf-resolvent}
  implies $\|(A-\sigma B)^{-1}B\| = \infty \geq 1/\epsilon$.

  Finally, to complete the cycle, we assume that
  $\|(A-\sigma B)^{-1}B\| \geq 1/\epsilon > 0$ and show that this implies the
  conditions of Definition~\ref{def:pseudo-def-pert1}.  If $\sigma = \infty$,
  then we must have $\sigma\in\Lambda_0(A)$, since otherwise by
  \eqref{eq:convention-infinite-shift} we would have
  $\|(A-\sigma B)^{-1}B\|=0$.  Thus $\sigma \in \Lambda_0(A-BE, B)$ for $E=0$
  with $\|E\|=0 \leq \epsilon$.  If $\sigma\in \C$ with
  $\sigma\in \Lambda_0(A,B)$, then we can also set $E=0$.  Suppose that
  $\sigma\in \C$ and $\sigma\notin \Lambda_0(A,B)$.  Then
  $\|(A-\sigma B)^{-1}B\|<\infty$ and from
  $\|(A-\sigma B)^{-1}B\| \geq 1/\epsilon$, we see that $\epsilon\neq 0$ and
  that there exists $\vec{v}$ such that $(A-\sigma B)^{-1}B \vec{v} = \vec{y}$
  with $\|\vec{y}\| \geq 1/\epsilon$ and $\|\vec{v}\| = 1$.  It follows that
  $(A-\sigma B) \vec{y} = B\vec{v}$.  Let $\|\cdot\|_*$ be the dual norm of
  $\|\cdot\|$.  If we have $\vec{x}$ with
  $\vec{x}^\H \vec{y} = 1 = \|\vec{x}\|_* \|\vec{y}\|$, then for
  $E = -\vec{v} \vec{x}^\H$ we have $(A-BE - \sigma B) \vec{y} = 0$ and
  $\|E\| = \|\vec{v}\| \|\vec{x}\|_* \leq \|\vec{v}\| /\|\vec{y}\| \leq
  \epsilon$.  The existence of $\vec{x}$ with the desired property is
  guaranteed by duality, \cite{hojo:85}.
\end{proof}
...
\end{equation*}
then we can restate \eqref{eq:pseudo-def-pert1} as
\begin{equation}
  \label{eq:pseudo-def-pert2}
  \Lambda_\epsilon(A,B) = \left\{ \sigma \in \widehat{\C} :
    \text{there exists $E\in \mcal{E}(A,B,\sigma)$ with $\|E\| \leq \epsilon$} \right\}.
\end{equation}
...
property that it may be empty.  This happens if there is no $E$ that makes
$\sigma$ a generalized eigenvalue of $(A-BE, B)$.  The analogous set for the
ordinary $\epsilon$-pseudospectrum of $A$ is $\{E : \text{$\sigma$ is an
  eigenvalue of $A-E$}\}$ and it is never empty.  The following corollary of
Theorem~\ref{th:def-equivalence} clarifies when $\mcal{E}(A,B,\sigma)$ is
empty.
\begin{corollary}
  \label{co:empty-E}
  The set $\mcal{E}(A,B,\sigma)$ is empty if and only if
  \begin{equation*}
    B=0\eqand \sigma \in \C
  \end{equation*}
  or
  \begin{equation*}
    \text{$B$ is nonsingular and $\sigma = \infty$}
  \end{equation*}
\end{corollary}
\begin{proof}
  From the equivalence of Definition~\ref{def:pseudo-def-pert1} and
  Definition~\ref{def:pseudo-resolvent} we know that $\mcal{E}(A,B,\sigma)$ is
  empty if and only if $\|(A-\sigma B)^{-1}B\|$ is zero.  Considering
  \eqref{eq:convention-inf-resolvent} and \eqref{eq:convention-infinite-shift},
  this can happen only in two cases: if \eqref{eq:convention-infinite-shift}
  applies, that is $\sigma=\infty$ and $B$ is nonsingular so that
  $\infty\notin\Lambda_0(A,B)$; or if \eqref{eq:convention-inf-resolvent} $B=0$
  and neither applies.
\end{proof}
...
\textit{pseudospectral separation} of $\sigma$ from $\Lambda_0(A,B)$.

The definition of $\delta(A,B,\sigma)$ gives another way to restate
\eqref{eq:pseudo-def-pert1}:
\begin{equation}
...
exists $E \in \mcal{E}(A,B,\sigma)$ with $\|E\| \leq \epsilon$ so that
$\delta(A,B,\sigma) \leq \|E\| \leq \epsilon$.  Conversely, if
$\delta(A,B,\sigma) = \epsilon_0 \leq \epsilon$, then $\mcal{E}(A,B,\sigma)$ is
nonempty and there exists $E_0\in\mcal{E}(A,B,\sigma)$ with
$\|E_0\| \leq \epsilon$ so that $\sigma \in \Lambda_\epsilon(A,B)$.  It is
also easily seen from this and Theorem~\ref{th:def-equivalence} that
\begin{equation*}
  \|(A-\sigma B)^{-1}B\| = \frac{1}{\delta(A,B,\sigma)}.
\end{equation*}

In the remainder of this paper we focus on $\|\cdot\| = \|\cdot\|_2$.  It is
known that $\sigma\in \C$ is in the $\epsilon$-pseudospectrum of $A$ when
$s_{\min}(A-\sigma I) \leq \epsilon$, where $s_{\min}$ denotes the minimum
singular value.  We do not emphasize this characterization of the generalized
$\epsilon$-pseudospectrum, but it can be shown that
$\sigma\in\Lambda_\epsilon(A,B)$ when the smallest generalized singular value
of $(A-\sigma B, B)$ is less than or equal to $\epsilon$.  In this case the
generalized singular value decomposition takes the form
\begin{equation*}
  A-\sigma B = S D_a U_a^\T, \eqand
  B = S D_b U_b^\T,
\end{equation*}
where $S$ is nonsingular, $U_a$ and $U_b$ are unitary, and $D_a$ and $D_b$ are
diagonal.  The generalized singular values are the diagonal values of
$D_b^{-1}D_a$ where a zero on the diagonal of $D_b$ corresponds to $\infty$ on
the diagonal of $D_b^{-1}D_a$.

If $B$ is nonsingular and $B^{-1}A$ is normal then it is easily seen that
\begin{equation*}
  \|(A-\sigma B)^{-1}\|_2 
  = \frac{1}{\delta_2(A,B,\sigma)} 
  = \frac{1}{\min_i|\lambda_i \sigma|},
\end{equation*}
where the minimum is taken over all generalized eigenvalues $\lambda_i$ of
$(A,B)$ and $\delta_2(A,B,\sigma)$ is $\delta(A,B,\sigma)$ defined with the
choice of 2-norm.  This also generalizes to the case in which $B$ is singular.
\begin{lemma}
  ...
  \begin{equation}
    ...
    \delta_2(A,B,\sigma) = \min_i |\lambda_i - \sigma|,
  \end{equation}
...
\begin{proof}
  ...
  $\delta_2(A,B,\sigma) = \|E_0\|_2 = |\lambda_1 -\sigma|$.
\end{proof}

For nonnormal matrices, the following generalizes a well known result for the
ordinary $\epsilon$-pseudospectrum \cite{trem:05}.
\begin{lemma}
...
\begin{proof}
  ...
  Lemma~\ref{lm:normal_eig_bound}, with $\delta_2(A,B,\sigma) = 0$ or
  $\delta_2(A,B,\sigma)=\infty$, depending on whether $(A,B)$ has an infinite
...
It is not particularly difficult to compute the generalized
$\epsilon$-pseudospectrum as defined here.  Computing
$\|(A-\sigma B)^{-1}B\|_2$ plays the part of the resolvent norm.  If we start
with a generalized Schur decomposition
\begin{equation}
  A = Q T_a Z^\H,\eqand
  B = Q T_b Z^\H,
\end{equation}
then one can efficiently apply the Lanczos algorithm to
\begin{equation*}
  Q^\H (A-\sigma B)^{-1}B B^\H (A-\sigma B)^{-\H} Q
  = (T_a - \sigma T_b)^{-1} T_b T_b^\H (T_a - \sigma T_b)^{-\H}
\end{equation*}
to compute $\|(A-\sigma B)^{-1}B\|_2$ with $\sigma$ chosen on a grid of points
in $\C$.  This is essentially what is often done for the ordinary
pseudospectrum with the ordinary Schur decomposition \cite[p. 375]{trem:05}.
We have used this approach to generate some of the graphs in
section~\ref{sec:numerical-examples}.

Finally, we note that much of what we have done here can be extended to the
case in which $A$ and $B$ are $m\times n$ with $m > n$.  However, in looking at
whether $\sigma$ is a generalized eigenvector of $(A-B E, B)$, the perturbation
$BE$ is constrained to be in the range of $B$.  While
Definition~\ref{def:pseudo-def-pert1} and Definition~\ref{def:pseudo-vector}
generalize readily, Definition~\ref{def:pseudo-resolvent} requires a more
substantial modification in which we introduce a pseudoinverse and look at the
norm of $(A-\sigma B)^{\dagger}B$ on a subspace
$\mcal{S} = \{ \vec{s} : B\vec{s} \in \mcal{R}(A-\sigma B)\}$, where
$\mcal{R}(X)$ denotes the range space of $X$.  Further, we are not aware of a
natural application for this sort perturbation.
...
\begin{equation*}
  K_2(A-\sigma B, B) = \frac{\|A-\sigma B\|_2}{\|B\|_2} \frac{1}{\delta_2(\sigma)}.
\end{equation*}
...
\begin{align}
  ...
  & = \|\hat{A} - \hat{\sigma} \hat{B}\|_2 
    \|(\hat{A} - \hat{\sigma} \hat{B})^{-1} \hat{B}\|_2 \nonumber\\
  & = \frac{\gamma}{\delta_2(\hat{A}, \hat{B}, \hat{\sigma})},\label{eq:K2-delta-ident}
\end{align}
...
\begin{equation*}
  K_2(A-\sigma B, B)
  \leq \frac{1+|\hat{\sigma}|}{\delta_2(\hat{A}, \hat{B}, \hat{\sigma})}
  = \frac{1+1/|\hat{\sigma}|}{\delta_2(\hat{A}, \hat{B}, \hat{\sigma})/|\hat{\sigma}|}
  = \frac{1+1/|\hat{\sigma}|}{\delta_2(A, B, \sigma)/|\sigma|}.
\end{equation*}
